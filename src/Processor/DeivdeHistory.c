...
{
    fprintf(dst, "ID\tДАТА\tИГРОК\tДЕЙСТВИЕ\tП0\tПРЕДМЕТ\tКОЛ-ВО\tОПИСАНИЕ\n");
}
...
)
{
    ...
    switch (record->type)
    {
        case 0:
            ...
            sprintf_s(description, description_size, "Игрок {role_id:%i} распределяет {item_id:%i} x%i",
                record->who,
                record->params[1],
                record->params[2]
            );
        ...
        case 1:
            ...
            sprintf_s(description, description_size, "Игрок {role_id:%i} получает {item_id:%i} x%i",
                record->who,
                record->params[1],
                record->params[2]
            );
        ...
        case 2:
            ...
            sprintf_s(description, description_size, "Игрок {role_id:%i} возвращает {item_id:%i} x%i",
                record->who,
                record->params[1],
                record->params[2]
            );
